...
def parse_contacts(vcffile, datafiles, output_dir, txt=False):
    with open(vcffile) as f:
        vcf_contacts = vobject.readComponents(f.read()) if txt is False else f.readlines()

    emails = {}
    for vcf_contact in vcf_contacts:
        if txt is False and 'email' not in vcf_contact.contents:
            continue
        name = vcf_contact.contents['fn'][0].value if txt is False else None
        if txt is False:
            ...
            email = Email(None, vcf_contact, datafiles, output_dir)
            emails.setdefault(email.initials, []).append(email)
    os.mkdir(output_dir)
    return emails
...
def main():
    ...
    emails = parse_contacts(args.vcf[0], datafiles, args.output_dir, txt=args.txt)
